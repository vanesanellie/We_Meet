...
class _Profil1State extends State<Profil1> {
  ...
  Future<void> getFeeds() async {
    ...
    var data = {"username": username};
    var response = await http.post(Uri.parse(url), body: (data));
    var feedResponse = json.decode(response.body);
    setState(() {
      feeds = feedResponse;
    });
    //print(feeds);
    
    // List<List> feedBytes1;
    // for(var i=0; i<feeds.length; i++){
    //   feedBytes1.add(base64.decode(feeds[i]['Image_Base64']));
    //   print(feedBytes1[i]);
    // }
    // setState(() {
    //   feedsBytes = feedBytes1;
    // });
  }
